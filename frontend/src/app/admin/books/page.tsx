...
export default function Books() {
    ...
    const currentQuotes = quotes.slice((quotePage - 1) * quotesPerPage, quotePage * quotesPerPage);
    ...
    const totalQuotePages = Math.ceil(quotes.length / quotesPerPage) || 1;
            ...
            <div className="flex-1 ml-64 p-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-between">
                        ...
                        {/* Genre Pagination Control */}
                        <div className="mt-6 flex justify-between items-center pt-4 border-t border-gray-50">
                            <span className="text-xs font-bold text-gray-400 uppercase">Page {genrePage} of {totalGenrePages}</span>
                            <div className="flex gap-2">
                                <button onClick={() => setGenrePage(p => Math.max(1, p - 1))} className="p-2 border rounded-lg hover:bg-gray-50"><ChevronLeft size={16}/></button>
                                <button onClick={() => setGenrePage(p => Math.min(totalGenrePages, p + 1))} className="p-2 border rounded-lg hover:bg-gray-50"><ChevronRight size={16}/></button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mt-8">
                    ...
                    {/* Quotes Pagination Control */}
                    <div className="mt-6 flex justify-between items-center">
                        <span className="text-xs font-bold text-gray-400 uppercase">Page {quotePage} of {totalQuotePages}</span>
                        <div className="flex gap-2">
                            <button onClick={() => setQuotePage(p => Math.max(1, p - 1))} className="p-2 border rounded-lg hover:bg-gray-50"><ChevronLeft size={18}/></button>
                            <button onClick={() => setQuotePage(p => Math.min(totalQuotePages, p + 1))} className="p-2 border rounded-lg hover:bg-gray-50"><ChevronRight size={18}/></button>
                        </div>
                    </div>
                </div>
