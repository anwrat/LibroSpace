...
import CharacterChatModal from "@/components/AiChat/CharacterChatModal";
...
export default function BookDetailsPage() {
  ...
  const [isChatOpen, setIsChatOpen] = useState(false); // Modal control state logic flag
  ...
  // Inline authorization logic assessment helper
  const hasChatAccess = user && user.level >= 4;
    ...
    async function fetchData() {
      try {
        ...
        const savedIds = new Set<number>(
          savedRes.data.data.map((q: any) => q.id),
        );
  ...
  const handleOpenChatModal = () => {
    if (!hasChatAccess) {
      toast.error(
        `Access Denied: Level 4 required! Your current rank level is ${user?.level || 0}.`,
      );
      return;
    }
    setIsChatOpen(true);
  ...
  if (!book)
    return <div className="pt-24 text-center font-main">Book not found</div>;
  ...
  return (
    <main className="min-h-screen pt-24 pb-20 px-6 max-w-7xl mx-auto font-main relative">
      ...
      <div className="flex flex-col md:flex-row gap-12">
        ...
        <div className="flex flex-col items-center gap-4 w-full max-w-[320px] mx-auto md:mx-0">
          ...
          {currentShelf === "currently_reading" && (
            <button
              ...
              disabled={updating}
              className="w-full flex items-center justify-center gap-3 bg-gray-900 text-white py-4 rounded-[1.5rem] font-bold hover:bg-[#14919B] transition-all shadow-xl hover:-translate-y-1 active:scale-95 disabled:opacity-70"
            >
              {updating ? (
          ...
          {/* Chat with Character Button Trigger */}
          <button
            onClick={handleOpenChatModal}
            className={`w-full flex items-center justify-center gap-3 py-4 rounded-[1.5rem] font-bold transition-all shadow-xl active:scale-95
                            ${
                              hasChatAccess
                                ? "bg-[#14919B] text-white hover:bg-[#0f7178] hover:-translate-y-1"
                                : "bg-gray-200 text-gray-400 cursor-pointer hover:bg-gray-300/80"
                            }
                        `}
          >
            {hasChatAccess ? (
            ...
            {!hasChatAccess && (
              <span className="text-[10px] bg-gray-300 text-gray-600 px-2 py-0.5 rounded-md ml-1 font-black">
                LVL 4
              </span>
            )}
          </button>
      ...
      <section className="mt-20">
        ...
        <button
          onClick={() => setAddQuotesModal(true)}
          disabled={updating}
          className="flex items-center gap-2 bg-[#14919B] text-white px-6 py-3 rounded-2xl font-bold hover:bg-[#0f7178] transition-all shadow-lg active:scale-95 disabled:opacity-50 mb-8"
        >
          <Plus size={22} />
          Add A Quote
        </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {quotes.map((quote) => {
            ...
            return (
              <div
                key={quote.id}
                className="group bg-white p-8 rounded-[2rem] border border-gray-100 shadow-sm hover:shadow-xl transition-all relative overflow-hidden"
              >
        ...
        {quotes.length === 0 && (
          <div className="text-center py-20 bg-gray-50 rounded-[2rem] border-2 border-dashed border-gray-200">
            <p className="text-gray-400 font-medium">
              No quotes shared for this book yet.
            </p>
          </div>
        )}
      </section>
        ...
        <div className="fixed inset-0 z-110 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-[2.5rem] p-10 w-full max-w-lg shadow-2xl animate-in zoom-in slide-in-from-bottom-4 duration-300">
            ...
            <p className="text-gray-500 text-sm mb-8">
              Your contribution will be reviewed by the community before
              appearing live.
            </p>
      ...
      {/* --- LIVE CHARACTER OVERLAY CHAT MODAL --- */}
      <CharacterChatModal
        isOpen={isChatOpen}
        onClose={() => setIsChatOpen(false)}
        bookTitle={book.title}
        bookAuthor={book.author}
        bookCover={book.cover_url || "/Placeholders/book-placeholder.png"}
      />
    </main>
  );
}
