...
export default function CommunityDetailsPage() {
  ...
  const [discSortType, setDiscSortType] = useState<"recent" | "popular">(
    "recent",
  );
  ...
  const handleRemoveMember = async (memberId: number, memberName: string) => {
    if (
      !confirm(
        `Are you absolutely sure you want to remove ${memberName} from this space?`,
      )
    )
      return;
    try {
    ...
    return members
      .filter((member) => {
        const matchesSearch = member.name
          ?.toLowerCase()
          .includes(memberSearchQuery.toLowerCase().trim());
        const matchesRole =
          selectedRoleFilter === "All" ||
          member.role?.toLowerCase() === selectedRoleFilter.toLowerCase();
  ...
  const currentSelectedBook = availableBooks.find(
      ...
      <main className="max-w-6xl mx-auto px-6 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {isMember && (
            <div
              className={`p-8 rounded-[2.5rem] border transition-all ${
                activeRoom
                  ? "bg-[#14919B] border-none text-white shadow-xl shadow-[#14919B]/30"
            ...
            >
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                  <div
                    className={`p-4 rounded-2xl ${activeRoom ? "bg-white/20 animate-pulse" : "bg-gray-100"}`}
                  >
                    <Radio
                      size={28}
                      className={activeRoom ? "text-white" : "text-gray-400"}
                    />
                  </div>
                  <div>
                    <h2
                      className={`text-xl font-black ${activeRoom ? "text-white" : "text-gray-900"}`}
                    >
                      {activeRoom ? "Live Reading Session" : "No Active Room"}
                    </h2>
                    <p
                      className={`text-sm ${activeRoom ? "text-white/80" : "text-gray-500"}`}
                    >
                      {activeRoom
                        ? `Reading: ${activeRoom.book_title}`
                        : "Start a session to read together with others."}
                    </p>
                  </div>
                </div>
                ...
                {activeRoom ? (
                  <button
                    onClick={() =>
                      router.push(`/user/community/${communityId}/live`)
                    }
                  ...
                  <div
                    className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto relative"
                            ...
                            filteredBooks.map((book) => (
                              ...
                              >
                                ...
                                {selectedBookId === String(book.id) && (
                                  <Check
                                    size={14}
                                    className="shrink-0 text-[#14919B]"
                                  />
                                )}
                              </button>
                            ))
                  ...
                  {sortedDiscussions.length > 0 ? (
                    sortedDiscussions.map((post) => (
                      <div
                        ...
                        onClick={() =>
                          router.push(
                            `/user/community/${communityId}/discussions/${post.id}`,
                          )
                        }
                      ...
                      >
                        <div>
                          <h3 className="text-xl font-black text-gray-900 mb-2 group-hover:text-[#14919B] transition-colors">
                            {post.title}
                          </h3>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-[2rem] p-6 border border-gray-100 shadow-xs">
            <h3 className="font-black text-gray-900 mb-3 text-base tracking-tight">
              About Space
            </h3>
            <p className="text-gray-500 text-sm leading-relaxed">
              {community?.description ||
                "A room engineered for reading analytics and cooperative book summaries."}
            </p>
          </div>
            ...
            <div className="bg-white rounded-[2rem] p-6 border border-gray-100 shadow-xs flex flex-col gap-4">
              <div>
                <h3 className="font-black text-gray-900 mb-1 text-base tracking-tight flex items-center gap-2">
                  <Users size={18} className="text-[#14919B]" /> Community
                  Members
                </h3>
              ...
              <div className="space-y-4 max-h-[380px] overflow-y-auto pr-1">
                {filteredAndSortedMembers.length > 0 ? (
                  filteredAndSortedMembers.map((member) => (
                    <div
                      key={member.id}
                      className="flex items-center justify-between gap-2 border-b border-gray-50/40 pb-2 last:border-none last:pb-0 animate-in fade-in duration-200"
                    >
                      <div className="flex items-center gap-2.5 min-w-0">
                        ...
                        <div className="min-w-0">
                          <Link
                            href={`/user/profile/${member.id}`}
                            className="text-xs font-black text-gray-900 leading-none truncate"
                          >
                            {member.name}
                          </Link>
