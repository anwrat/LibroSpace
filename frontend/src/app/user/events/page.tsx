  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (joined) {
        ...
        setListings(
          allListings.filter((l: any) => l.user_id !== currentUser?.id),
        );
        setMyListings(
          allListings.filter((l: any) => l.user_id === currentUser?.id),
        );
      ...
      <div className="max-w-7xl mx-auto">
        {!hasJoined ? (
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
            <header className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
              <div>
                ...
                <p className="text-gray-500 font-bold">
                  Swap your stories with the world.
                </p>
              </div>
            ...
            {activeTab === "requests" && (
              <RequestsTab requests={requests} refresh={fetchData} />
            )}
            {activeTab === "my-books" && (
              <MyBooksTab
...
function TabButton({ active, onClick, icon, label }: any) {
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-2 px-6 py-3 rounded-[1.2rem] text-xs font-black uppercase tracking-widest transition-all ${active ? "bg-white text-[#14919B] shadow-sm scale-[1.02]" : "text-gray-400 hover:text-gray-600"}`}
    >
      {icon} {label}
    </button>
  );
...
function LoadingScreen() {
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-white">
      ...
      <p className="font-black text-[#14919B] italic animate-pulse">
