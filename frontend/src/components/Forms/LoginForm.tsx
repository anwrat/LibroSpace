...
export default function LoginForm() {
  ...
  async function handlelogin(e: React.FormEvent) {
    ...
    try {
      ...
      // Use window.location.href to guarantee an absolute structural reload for auth layout contexts
  ...
  return (
    <form onSubmit={handlelogin} className="flex flex-col gap-6 w-full">
      ...
      <div>
        <h1 className="font-main font-black text-4xl text-gray-900 tracking-tight">Log In</h1>
        <p className="text-gray-400 text-sm font-medium mt-1.5">Access your interactive personal library notebook.</p>
      </div>
      ...
      <div className="flex flex-col gap-2 mt-2">
        <div className="flex items-center gap-3 px-4 py-3.5 bg-gray-50 border border-gray-200/80 rounded-2xl focus-within:border-[#14919B] focus-within:bg-white focus-within:ring-4 focus-within:ring-[#14919B]/10 transition-all duration-200 group">
          <User size={20} className="text-gray-400 group-focus-within:text-[#14919B] transition-colors" />
          <input 
            type="text" 
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-3 px-4 py-3.5 bg-gray-50 border border-gray-200/80 rounded-2xl focus-within:border-[#14919B] focus-within:bg-white focus-within:ring-4 focus-within:ring-[#14919B]/10 transition-all duration-200 group">
          <Lock size={20} className="text-gray-400 group-focus-within:text-[#14919B] transition-colors" />
          <input 
        ...
        <div className="flex items-center justify-between">
          <button 
            type="button"
            onClick={() => { /* Handle missing password routing here */ }}
      ...
      {/* Action Submit Button Component Alternative */}
      <button 
        type="submit" 
        disabled={isSubmitting}
      ...
      {/* Alternative Login Providers Divider Interface */}
      <div className="flex flex-col items-center gap-4 mt-2">
        <div className="flex items-center w-full gap-3 my-1">
          <div className="h-px bg-gray-200 flex-1" />
          <span className="font-main text-xs font-black text-gray-400 tracking-widest uppercase">Or Protocol</span>
          <div className="h-px bg-gray-200 flex-1" />
        </div>
