import { Inbox, Send, Check, X, Repeat } from "lucide-react";
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-[2.5rem] p-8 shadow-sm">
      <h3 className="font-black text-gray-900 uppercase tracking-widest text-sm mb-8 italic flex items-center gap-2">
        {type === "sent" ? <Send size={16} /> : <Inbox size={16} />} {title}{" "}
        Proposals
      </h3>
      <div className="space-y-4">
        {data.length > 0 ? (
          data.map((item: any) => (
            <div
              key={item.id}
              className="p-6 bg-gray-50/50 rounded-[2rem] border border-transparent hover:border-gray-100 transition-all"
            >
              <div className="flex items-center justify-between mb-4">
                ...
                {type === "received" && item.status === "pending" && (
                  <div className="flex gap-2">
                    <button
                      onClick={() => onResponse(item.id, "accepted")}
                      className="p-2 bg-[#14919B] text-white rounded-lg"
                    >
                      <Check size={16} />
                    </button>
                    <button
                      onClick={() => onResponse(item.id, "rejected")}
              ...
              <div className="flex justify-between items-center text-[10px] font-bold uppercase text-gray-400">
                <span>
                  {type === "received" ? item.sender_name : item.owner_name}
                </span>
                <span
                  className={item.status === "accepted" ? "text-green-500" : ""}
                >
                  {item.status}
                </span>
              </div>
          ...
          <p className="text-center py-10 text-gray-300 font-black italic uppercase text-xs">
            Empty
          </p>
        )}
      </div>
