...
import { insertDailyGoalAchievement, checkGoalMetYesterday, getDailyGoalsAchievedDateAllTime, checkIfGoalAlreadyAchieved } from "../../models/gamification/daily_goals_achieved.model.js";
...
export const getAchievementAllTime = async(req: Request, res: Response) =>{
    try{
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ message: "Unauthorized" });
        }
        const achievedDates = await getDailyGoalsAchievedDateAllTime(userId);
