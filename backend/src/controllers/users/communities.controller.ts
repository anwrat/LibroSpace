...
} from "../../models/communities/communities.model.js";
import {
...
import {
  ...
  getDiscussionById,
  deleteDiscussion,
} from "../../models/communities/discussions.model.js";
import {
  addComment,
  getCommentsbyDiscussionId,
  deleteComment,
} from "../../models/communities/comments.model.js";
import {
  assignRoleToMember,
  checkMemberRole,
  getAllMembersByCommunityId,
  removeMemberFromCommunity,
} from "../../models/communities/community_members.model.js";
import {
  getActiveRoomsByCommunityId,
  startNewRoom,
  endRoom,
} from "../../models/communities/community_rooms.model.js";
import { awardActivityXP } from "../../services/gamification.service.js";

export const checkCommunityExistence = async (req: Request, res: Response) => {
  try {
    const { name } = req.body;
    const existingCommunity = await checkIfCommunityExists(name);
    if (existingCommunity.length > 0) {
      return res.status(400).json({
        exists: true,
        message: "Community with the same name already exists",
      });
    }
    ...
    console.error(err);
    res.status(500).json({
      message: "Internal Server Error while checking community existence",
    });
  }
};
...
export const addNewCommunity = async (req: Request, res: Response) => {
  try {
    ...
    return res.status(201).json({
      ...
      details: newCommunity,
    });
  } catch (err) {
    console.error(err);
    res
      .status(500)
      .json({ message: "Internal Server Error while adding new community" });
  }
};
...
export const fetchAllCommunities = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Internal Server Error while fetching communities" });
  }
};
...
export const fetchJoinedCommunities = async (req: Request, res: Response) => {
  try {
    ...
    console.error(err);
    res.status(500).json({
      message: "Internal Server Error while fetching joined communities",
    });
  }
};
...
export const getAllDiscussionsByCommunityId = async (
  req: Request,
  res: Response,
) => {
  try {
...
export const getDiscussionDetailsById = async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Internal Server Error while fetching discussion details",
    });
  }
};
...
export const deleteADiscussion = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "Discussion deleted successfully" });
  } catch (err) {
    console.error(err);
    res.status(500).json({
      success: false,
...
export const addCommentToDiscussion = async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Internal Server Errror while adding comment",
    });
  }
};
...
export const deleteACommentfromDiscussion = async (
  req: Request,
  res: Response,
) => {
  try {
...
export const getAllMembersByCommunity = async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Internal Server Error while fetching community members",
    });
  }
};
  ...
  try {
    ...
    if (userRole !== "mentor") {
      return res.status(403).json({
        success: false,
        message: "Only mentor can change member roles",
      });
    }
    ...
    res.status(500).json({
      success: false,
      message: "Internal Server Error while changing member role",
    });
  }
};
...
export const removeMember = async (req: Request, res: Response) => {
  try {
    ...
    if (removerRole !== "mentor") {
      return res
        .status(403)
        .json({ success: false, message: "Only mentor can remove members" });
    }
    ...
    res.status(500).json({
      success: false,
      message: "Internal Server Error while removing member from community",
    });
  }
};
...
export const getActiveRoom = async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Internal Server Error while fetching active rooms",
    });
  }
};
...
export const startRoom = async (req: Request, res: Response) => {
  try {
    ...
    return res.status(201).json({
      success: true,
      message: "New room started successfully",
      data: room,
    });
  } catch (err) {
    console.error(err);
    res.status(500).json({
      success: false,
      message: "Internal Server Error while starting a new room",
    });
  }
};
...
export const endActiveRoom = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "Room ended successfully", data: room });
  } catch (err) {
    console.error(err);
    res.status(500).json({
      success: false,
      message: "Internal Server Error while ending the room",
    });
  }
};
