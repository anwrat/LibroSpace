...
export const findBookByTitleandAuthor = async (
  title: string,
  author: string,
) => {
...
export const getCurrentBookCover = async (id: number) => {
  const currentBookQuery = "SELECT cover_url FROM books.booklist WHERE id = $1";
  const { rows } = await pool.query(currentBookQuery, [Number(id)]);
  return rows.length > 0 ? rows[0].cover_url : null;
};
...
export const updateBookDetails = async (
  ...
  try {
    ...
    await client.query("DELETE FROM books.book_genres WHERE book_id = $1", [
      id,
    ]);
    ...
    if (genreIds && genreIds.length > 0) {
      ...
      await client.query(
        `INSERT INTO books.book_genres (book_id, genre_id) VALUES ${values}`,
        [id, ...genreIds],
      );
    }
